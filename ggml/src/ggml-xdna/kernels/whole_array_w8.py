...
ggml-xdna variant of mlir-aie's programming_examples/basic/matrix_multiplication/whole_array/whole_array.py:
A holds int8 weight rows, B bf16 activation rows (column-major), C f32 (column-major), and the
compute tiles run ``mm_w8.cc`` (next to this file), which widens A to bf16 in registers. Only
the element types and the kernel binding differ from the upstream design; the data movement is
unchanged.
...
_W8_KERNEL_SOURCE = Path(__file__).resolve().parent / "mm_w8.cc"
...
def _build_design(
    ...
    *,
    ...
    # int8 weight rows (A) x bf16 activation rows (B) -> f32 (C), in ggml-xdna's layouts; the
    # dtype_in_str / dtype_out_str / emulate / chess / scalar arguments are ignored
    dtype_a = np.int8
    dtype_b = bfloat16
    dtype_out = np.float32
    assert b_col_maj and c_col_maj, "mm_w8.cc supports column-major B and C only"

    r, s, t = 4, 8, 4   # the bf16 mmul of mm_w8.cc
    matmul_kernel = _make_extern(
        "matmul_i8bf16_f32",
        _W8_KERNEL_SOURCE,
        [
            np.ndarray[(m * k,), np.dtype[dtype_a]],
            np.ndarray[(k * n,), np.dtype[dtype_b]],
            np.ndarray[(m * n,), np.dtype[dtype_out]],
        ],
        compile_flags=[f"-DDIM_M={m}", f"-DDIM_K={k}", f"-DDIM_N={n}", "-DB_COL_MAJ", "-DC_COL_MAJ"],
    )
    zero_kernel = Kernel(
        "zero_f32",
    ...
    A_ty = np.ndarray[(M * K,), np.dtype[dtype_a]]
    B_ty = np.ndarray[(K * N,), np.dtype[dtype_b]]
    C_ty = np.ndarray[(M * N,), np.dtype[dtype_out]]
    A_l2_ty = np.ndarray[(m * k * n_A_tiles_per_shim,), np.dtype[dtype_a]]
    B_l2_ty = np.ndarray[(k * n,), np.dtype[dtype_b]]
    C_l2_ty = np.ndarray[(m * n * n_aie_rows,), np.dtype[dtype_out]]
    A_l1_ty = np.ndarray[(m, k), np.dtype[dtype_a]]
...
def _make_argparser():
    ...
    p.add_argument("--b-col-maj", type=int, choices=[1], default=1)
    p.add_argument("--c-col-maj", type=int, choices=[1], default=1)
    p.add_argument(
...
def _run_and_verify(opts):
    ...
    A_np = rng.integers(-127, 128, size=(opts.M, opts.K), dtype=np.int8)
    B_np = (rng.random((opts.N, opts.K)) * 4.0).astype(bfloat16)
    A_t = iron.tensor(A_np, dtype=np.int8, device="npu")
    ...
    expected_logical = (A_np.astype(np.float32) @ B_np.astype(np.float32).T).astype(dtype_out)
    if opts.c_col_maj:
